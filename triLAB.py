...
def sortL(profiles:list[dict]):
    sortedProfiles=[]
    while profiles:
        minL=float('infinity')
        minProfiles=None
        for profile in profiles:
            if profile['LAB'][0]<minL:
                minL=profile['LAB'][0]
                minProfiles=profile
        sortedProfiles.append(minProfiles)
        profiles.remove(minProfiles)
...
def organisedGrid(profiles: list[dict], clusters:dict):
    ...
    for centroid in sortedCentroids:
        sortedProfiles=sortL([profile for profile in profiles if 'LAB Cluster' in profile and profile['LAB Cluster'] == centroid['LAB']])
