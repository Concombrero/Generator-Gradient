...
def dataLAB(profiles: list[dict])->np.array:
    """Créer la data des codes LAB à partir d'une liste de profil d'image. On ne garde alors que les composantes (a,b)
    ...
    for profile in profiles:
        codeLAB=profile['LAB']
        codeArrayLAB=np.array([codeLAB[1], codeLAB[2]])
        data.append(codeArrayLAB)
    return np.array(data)
